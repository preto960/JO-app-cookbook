...
import {
  authService, storage, clearTokens, registerAuthExpiredListener, setAccessTokenMemory,
} from '../services/api';
...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    (async () => {
      try {
        ...
        try {
          ...
          const isInvalidToken = status === 401 || (status === 403 && (message.includes('invalid token') || message.includes('jwt')));
          if (isInvalidToken) {
  ...
  const login = async (email: string, password: string) => {
    ...
    if (email.trim().toLowerCase() === SUPERADMIN_EMAIL && password === SUPERADMIN_PASSWORD) {
      ...
      setAccessTokenMemory('local-superadmin-token');
